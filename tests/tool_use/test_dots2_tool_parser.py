...
from vllm.entrypoints.openai.tool_parsers import Dots2ToolParser
from vllm.transformers_utils.tokenizer import get_tokenizer

# Use a common model that is likely to be available
MODEL = "/cpfs/user/taishan/model/rednote-hilab/dots2.llm.inst.fp8"
...
@pytest.fixture
def dots2_tool_parser(dots2_tokenizer):
    return Dots2ToolParser(dots2_tokenizer)
...
def test_extract_tool_calls_no_tools(dots2_tool_parser):
    model_output = "This is a test"
    extracted_tool_calls = dots2_tool_parser.extract_tool_calls(
...
{"name": "get_current_weather", "arguments": {"city": "Orlando", "state": "FL", "unit": "fahrenheit"}}
        ...
        (
            """I'll help you check the weather. <dots_function_call>
...
def test_extract_tool_calls(dots2_tool_parser, model_output,
...
@pytest.mark.skip(reason="TODO: thinking tags are not supported yet")
def test_preprocess_model_output_with_thinking_tags(dots2_tool_parser):
    """Test that tool calls within thinking tags are removed during preprocessing."""
    model_output = """<think>Let me think about this. <dots_function_call>
{"name": "fake_tool", "arguments": {"param": "value"}}
</dots_function_call> This should be removed.</think>

I'll help you with that. <dots_function_call>
{"name": "get_current_weather", "arguments": {"city": "Seattle", "state": "WA"}}
</dots_function_call>"""

    processed_output = dots2_tool_parser.preprocess_model_output(model_output)
...
@pytest.mark.skip(reason="TODO: thinking tags are not supported yet")
def test_extract_tool_calls_with_thinking_tags(dots2_tool_parser):
    """Test tool extraction when thinking tags contain tool calls that should be ignored."""
    model_output = """<think>I should use a tool. <dots_function_call>
{"name": "ignored_tool", "arguments": {"should": "ignore"}}
</dots_function_call></think>

Let me help you with the weather. <dots_function_call>
{"name": "get_current_weather", "arguments": {"city": "Miami", "state": "FL", "unit": "fahrenheit"}}
    ...
    # Content extraction is based on the position of the first <dots_function_call> in the original model_output
    # Since preprocessing removes tool calls within thinking tags, the actual first <dots_function_call> is the external one
    expected_content = """<think>I should use a tool. <dots_function_call>
{"name": "ignored_tool", "arguments": {"should": "ignore"}}
</dots_function_call></think>
...
@pytest.mark.skip(reason="TODO: invalid JSON is not supported yet")
def test_extract_tool_calls_invalid_json(dots2_tool_parser):
    """Test that invalid JSON in tool calls is handled gracefully."""
    model_output = """<dots_function_call>
...
@pytest.mark.skip(
    reason="TODO: missing name or arguments is not supported yet")
def test_extract_tool_calls_missing_name_or_arguments(dots2_tool_parser):
...
def test_streaming_basic_functionality(dots2_tool_parser):
    """Test basic streaming functionality."""
    # Reset streaming state
    dots2_tool_parser.current_tool_name_sent = False
    dots2_tool_parser.prev_tool_call_arr = []
    dots2_tool_parser.current_tool_id = -1
    dots2_tool_parser.streamed_args_for_tool = []

    # Test with a simple tool call
    current_text = """<dots_function_call>
{"name": "get_current_weather", "arguments": {"city": "Seattle"}}
</dots_function_call>"""

    # First call should handle the initial setup
    result = dots2_tool_parser.extract_tool_calls_streaming(
        previous_text="",
        current_text=current_text,
        delta_text="</dots_function_call>",
        previous_token_ids=[],
        current_token_ids=[],
        delta_token_ids=[],
        request=None,
...
def test_streaming_with_content_before_tool_calls(dots2_tool_parser):
...
def test_streaming_no_tool_calls(dots2_tool_parser):
...
def test_streaming_with_thinking_tags(dots2_tool_parser):
    ...
    current_text = """<think><dots_function_call>{"name": "ignored", "arguments": {}}</dots_function_call></think><dots_function_call>{"name": "real_tool", "arguments": {"param": "value"}}</dots_function_call>"""
...
def test_extract_tool_calls_multiline_json(dots2_tool_parser):
    """Test that multiline JSON in tool calls."""
    model_output = """<dots_function_call>
{
  "name": "get_current_weather",
  "arguments": {
...
def test_streaming_arguments_incremental_output(dots2_tool_parser):
...
def test_streaming_arguments_delta_only(dots2_tool_parser):
    ...
    result1 = dots2_tool_parser.extract_tool_calls_streaming(
        previous_text="",
    ...
    result2 = dots2_tool_parser.extract_tool_calls_streaming(
...
@pytest.mark.skip(reason="TODO: streaming is not supported yet")
def test_streaming_openai_compatibility(dots2_tool_parser):
    ...
    dots2_tool_parser.pending_buffer = ""
    dots2_tool_parser.in_thinking_tag = False
    dots2_tool_parser.thinking_depth = 0
    ...
    test_cases: list[dict[str, Any]] = [
        ...
        },
        {
            ...
            'current': 'Text content</dots_function_call>',
            'delta': 'calls>',
...
@pytest.mark.skip(reason="TODO: streaming is not supported yet")
def test_streaming_thinking_tag_buffering(dots2_tool_parser):
    ...
    test_cases: list[dict[str, Any]] = [
        {
            ...
            '',
            'current':
            '<think>I need to use a tool. <dots_function_call>',
            'delta':
            '<think>I need to use a tool. <dots_function_call>',
            'expected_content':
            '<think>I need to use a tool. <dots_function_call>',  # Should pass through as content
        },
        {
            'stage':
            ...
            '\n{"name": "ignored_tool", "arguments": {"param": "value"}}\n</dots_function_call>',
            'expected_content':
            '\n{"name": "ignored_tool", "arguments": {"param": "value"}}\n</dots_function_call>',  # </dots_function_call> should be preserved in thinking tags
        },
        {
            ...
            'delta': '\n<dots_function_call>',
            'expected_content':
            '\n',  # Should output '\n' and suppress <dots_function_call>
        }
    ]
...
@pytest.mark.skip(reason="TODO: streaming is not supported yet")
def reset_streaming_state(dots2_tool_parser):
    """Helper function to properly reset the streaming state for Dots2ToolParser."""
    # Reset minimax-specific state
    dots2_tool_parser._reset_streaming_state()
...
@pytest.mark.skip(reason="TODO: streaming is not supported yet")
def test_streaming_complex_scenario_with_multiple_tools(dots2_tool_parser):
    """Test complex streaming scenario: tools inside <think> tags and multiple tool calls in one group."""
    # Reset streaming state
    reset_streaming_state(dots2_tool_parser)
    ...
    test_stages: list[dict[str, Any]] = [
        ...
        },
        {
            ...
            'Let me help you with this task.<think>I need to analyze this situation first.',
            'current':
            'Let me help you with this task.<think>I need to analyze this situation first.<dots_function_call>',
            'delta': '<dots_function_call>',
            'expected_content':
            '<dots_function_call>',  # Inside thinking tags, tool tags should be preserved as content
            'expected_tool_calls': 0,
        },
        {
        ...
        },
        {
            ...
            'Let me help you with this task.<think>I need to analyze this situation first.<dots_function_call>\n{"name": "internal_analysis", "arguments": {"query": "analyze situation"}}\n</dots_function_call></think>',
            'current':
            'Let me help you with this task.<think>I need to analyze this situation first.<dots_function_call>\n{"name": "internal_analysis", "arguments": {"query": "analyze situation"}}\n</dots_function_call></think>\nNow I need to get weather information and calculate area.<dots_function_call>',
            'delta':
            '\nNow I need to get weather information and calculate area.<dots_function_call>',
            'expected_content':
            '\nNow I need to get weather information and calculate area.',  # <dots_function_call> should be filtered
            'expected_tool_calls': 0,
        },
        {
        ...
        },
        {
            ...
            'Let me help you with this task.<think>I need to analyze this situation first.<dots_function_call>\n{"name": "internal_analysis", "arguments": {"query": "analyze situation"}}\n</dots_function_call></think>\nNow I need to get weather information and calculate area.<dots_function_call>\n{"name": "get_current_weather", "arguments": {"city": "Seattle", "state": "WA", "unit": "celsius"}}\n{"name": "calculate_area", "arguments": {"shape": "rectangle", "dimensions": {"width": 10, "height": 5}}}',
            'current':
            'Let me help you with this task.<think>I need to analyze this situation first.<dots_function_call>\n{"name": "internal_analysis", "arguments": {"query": "analyze situation"}}\n</dots_function_call></think>\nNow I need to get weather information and calculate area.<dots_function_call>\n{"name": "get_current_weather", "arguments": {"city": "Seattle", "state": "WA", "unit": "celsius"}}\n{"name": "calculate_area", "arguments": {"shape": "rectangle", "dimensions": {"width": 10, "height": 5}}}</dots_function_call>',
            'delta': '</dots_function_call>',
            'expected_content': None,
            'expected_tool_calls': 0,
        }
...
@pytest.mark.skip(reason="TODO: streaming is not supported yet")
def test_streaming_character_by_character_output(dots2_tool_parser):
    ...
    complete_text = """I'll help you with the weather analysis. <think>Let me think about this. <dots_function_call>
{"name": "internal_analysis", "arguments": {"type": "thinking"}}
</dots_function_call>This tool should be ignored.</think>

Now I'll get the weather information for you. <dots_function_call>
{"name": "get_current_weather", "arguments": {"city": "Seattle", "state": "WA", "unit": "celsius"}}
{"name": "calculate_area", "arguments": {"shape": "rectangle", "dimensions": {"width": 10, "height": 5}}}
</dots_function_call>Here are the results."""
    ...
    for i in range(1, len(complete_text) + 1):
        ...
        if result is not None:
            if hasattr(result, 'content') and result.content:
                ...
                if any(keyword in result.content for keyword in [
                        '<think>', '</think>', '<dots_function_call>',
                        '</dots_function_call>'
                ]):
                    print(
    ...
    # Verify that <dots_function_call> and </dots_function_call> tags are not included in the final content
    ...
    tool_calls_in_content = content_outside_thinking.count(
        '<dots_function_call>')
    assert tool_calls_in_content == 0, f"<dots_function_call> tags should be filtered from content outside thinking tags, but found {tool_calls_in_content}"
...
@pytest.mark.skip(reason="TODO: streaming is not supported yet")
def test_streaming_character_by_character_simple_tool_call(dots2_tool_parser):
    """Test character-by-character streaming for a simple tool call scenario."""
    # Reset streaming state
    reset_streaming_state(dots2_tool_parser)

    # Simple tool call text
    simple_text = 'Let me check the weather. <dots_function_call>\n{"name": "get_weather", "arguments": {"city": "NYC"}}\n</dots_function_call>'
...
@pytest.mark.skip(reason="TODO: streaming is not supported yet")
def test_streaming_character_by_character_with_buffering(dots2_tool_parser):
    ...
    buffering_text = 'Hello world<dots_function_call>\n{"name": "test"}\n</dots_function_call>done'
    ...
    # The parser should handle the edge case where </dots_function_call> appears before <dots_function_call>
